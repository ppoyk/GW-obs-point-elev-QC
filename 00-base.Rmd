...
# Kirjastojen asennus hoituu {renv} kautta
# Tarvittaessa renv::init()
...
if (!requireNamespace("here", quietly = TRUE)) {
  warning("Package \"here\" must be installed to execute.", call. = FALSE)
  install.packages("here", destdir = "libs", lib = "libs")
}
...
pkgs <- c("rmarkdown","ggplot2","ggstatsplot","ggrepel","ggridges","here",
          "lidR","progress","rlas","gstat","sf","terra","dplyr","stringr",
          "data.table","yaml","rnaturalearth","rnaturalearthdata","rlang",
          "DescTools","xfun","here","profvis","vctrs","scales","caret",
          "knitr","remotes","DBI","rstudioapi","assertthat","tidyr",
...
if (!exists("kp", inherits = F)) source(file.path(D$koodi,"tuo_kiintopisteet.R"))
...
```{r LAS-aineisto-korkeusmalleiksi-ja-korkeuden-lukeminen}
...
rm(a, b, r, bwid, xlims, lon_err, lat_err)
```
...
for (i in 1:nrow(kaikki_paikat)) {
  ...
  if (!is.na(kaikki_paikat[i,"yhd_putki"])) {
    ...
    # Also check that length is positive(>10cm) and not exactly 1m (oft-used paceholder)
    if (kaikki_paikat[i,"db_putki"] > 0.1 && kaikki_paikat[i,"db_putki"] != 1.000) {
...
for (i in 1:nrow(kaikki_paikat)) {
  ...
  yla<-NA; yla_source<-NA; yla_verif<-NA # New value of the pipe top elevation
  maa<-NA; maa_source<-NA; maa_verif<-NA # New value of the ground elevation
...
stopifnot(!anyNA(putki_yhd_temp$yla)) # All elevs filled
stopifnot(!anyNA(putki_yhd_temp$maa))
stopifnot(!anyNA(putki_yhd_temp$yla_source)) # All sources filled
stopifnot(!anyNA(putki_yhd_temp$maa_source))
stopifnot(all(putki_yhd_temp$maa > -5)) # All ground elevs > -5m
stopifnot(all(putki_yhd_temp$yla < 1300)) # All pipe tops <1300m
stopifnot(all(putki_yhd_temp$yla - putki_yhd_temp$maa > 0.1)) #Visible sections >10cm
stopifnot(all(putki_yhd_temp$yla - putki_yhd_temp$maa < 2)) #Visible sections <2m
stopifnot(all(putki_yhd_temp$yla > putki_yhd_temp$maa)) #All tops above ground
...
``` {r clean-&-save-elevation-table}

# Remove top elevs for locations other than wells and monitoring pipes
for (i in 1:nrow(putki_yhd_temp)) {
  # Check that type is NOT monitoring pipe or well
  if (!putki_yhd_temp[i, "PaikkaTyyppi_Id"] %in% c("Havaintoputki","Kaivo")) {
...
# TODO Ensure that pairs of manual and autom. pipes have the same elev values
link_am_man <- hae_PaikkaAutomaattiManuaali()
vertaa_am_man <- data.frame(matrix(nrow=nrow(link_am_man), ncol=10))
for (i in 1:nrow(link_am_man)) {
  ...
  p_a <- putki_yhd_temp[putki_yhd_trim$paikka_id == a_id, ]
  p_m <- putki_yhd_temp[putki_yhd_trim$paikka_id == m_id, ]
  ...
  # If top elevs already equal, ensure that ground elevs are equal too
    ...
    if (isTRUE(all.equal(p_a$maa, p_m$maa))) {
        vertaa_am_man[i, ] <- NA
    } else if (sum(c(p_a$maa_source, p_m$maa_source) =="DB") == 1) {
      # Get the ground value to be used for both (am & man) places
      new_maa <- eq_tester(p_a, p_m, "maa_source",should_eq="DB", out_var="maa")
      # Assign to both (one already is correct, but advanced logic can be omitted)
      putki_yhd_temp[putki_yhd_temp$paikka_id == a_id, "maa"] <- new_maa
      putki_yhd_temp[putki_yhd_temp$paikka_id == m_id, "maa"] <- new_maa 

      message(paste(eq_tester(p_a, p_m, "maa_source",should_eq="DB", out_var="maa"),
                eq_tester(p_a, p_m, "maa_source",should_eq="DB", out_var="tunnus")))
      
    }
  } else {
    vertaa_am_man[i, ] <- c(p_a$tunnus, p_m$tunnus,
                          p_a$yla, p_a$yla_source, p_m$yla, p_m$yla_source,
                          p_a$maa, p_a$maa_source, p_m$maa, p_m$maa_source)
  } 
...
colnames(vertaa_am_man) <- c("tunnus_AM","tunnus_man",
                             "AM_yla","AM_y_source","man_yla","man_y_source",
                             "AM_maa","AM_m_source","man_maa","man_m_source")
vertaa_am_man <- vertaa_am_man[!is.na(vertaa_am_man$tunnus_AM), ] #drop NA rows
rm(a_id,m_id, p_a,p_m, new_maa)


# Drop rows where man and autom. have matching elevs (0 diff)


# Drop suspected bad ELY measurements (later possibly replaced with DTM and KM2 values)
#bad_ely_elev <- putki_yhd_temp[putki_yhd_temp$yla_verif %in% c("ELY_IN_DB?"), ]
# TODO jos ELYn mittauksen vertailutulokset ovat huonot , siirrytäänkö suoraan
# tarkistamaan korkeusmallien tarkastukseen, eikä oteta ELYä huomioon?

# Drop also stations which have potentially other members/organisations utilizing
# the observations. (Old VHSP-stations)


# Also drop structures relating to some waterworks.


# Drop select VARELY autom.devices.(observations are uploaded via custom process)
putki_yhd_temp <- putki_yhd_temp[!putki_yhd_temp$paikka_id %in%
                                   c(52191, 52845, 54763, 59979, 59983, 60177,
                                     59788, 62327, 66770, 74869, 75020, 75021), ]
# Drop quality monitoring pipes at most stations
# (gw elevs sent wrong, timeseries would break if top elev changes)
# (should include SeSu places)
putki_yhd_temp <- putki_yhd_temp[
  !putki_yhd_temp$paikka_id %in%
    c(7781, 7775, 7779, 41028, 3453, 3454, 2615, 2616, 3455, 3456, 2617, 9167,
      9169, 5006, 4208, 7769, 11739, 40032, 40017, 37314, 78135, 7768, 47180,
      54272, 88516, 82298, 45946, 14522, 69447, 58880, 88501, 7777, 7772, 7764,
      40914, 42279, 64688, 40143, 39192, 39445, 14491, 11583, 7836, 7833, 49839,
      7853, 7850, 7827, 14369, 14371, 21719, 69776, 7830, 47492, 71342, 73700,
      75674, 7747, 7861, 7862, 7817, 7735, 7732, 41512, 43463, 53120, 75435,
      7816, 7808, 7802, 17997, 7740, 3839, 24911, 7133, 7174, 7126, 69436,
      88424, 88425), ]
...
# TODO drop asemat joissa ei epäilyttäviä linkityksiä

# Ei käsittelyyn mukaan vielä ELYjen mittausten vertailua 2022 tilanteeseen, eikä
# niiden tapausten käsittelyä, joissa ELYjen mittaukset toleranssien ulkopuolella muista malleista
# Kaikki ELYn mittaukset joissa sekä yläpää että maanpinta täsmää tietokantaan, pudotetaan pois



test <-
  putki_yhd_temp[,c("paikka_id","asema_tunnus","asema_nimi","tunnus","PaikkaTyyppi_Id",
...
putki_korkeudet_outfile <-
  putki_yhd_temp[ ,c("paikka_id","tunnus","asema_tunnus","asema_nimi",
                     "PaikkaTyyppi_Id","yla","yla_source","yla_verif",
                     "maa","maa_source","maa_verif","KM2_laatu")]

#Round numeric cols to two decimal digits
putki_korkeudet_outfile <-
  dplyr::mutate_if(putki_korkeudet_outfile, is.numeric, ~round(., 2)) |>
  dplyr::mutate(PaikkaTyyppi_Id=as.num(PaikkaTyyppi_Id))

# Write the results to a file
data.table::fwrite(putki_korkeudet_outfile,
  check_dir(D$tulokset,"korkeudet","korkeudet_tietokantaan_DRAFT.csv", ensure = T),
  bom = T)
...
```{r session-info}
writeLines(capture.output(sessionInfo()), f.path("doc","R_session_info.txt"))
```
