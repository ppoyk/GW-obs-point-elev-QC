...
mas_aj_2_asemat <- c("0104", "0106", # Tullinkangas, Keräkankare
                     "0110", "0202", # Pernaja-Koskenkylä, Perniö
                     "0501", "0504", "0508", # Pertunmaa, Särkelä, Raikuunkangas
                     "0604", "1110", "1301") #  Akonjoki, Rokua, Könölä
asemataulu <- asemataulu[asemataulu$tunnus %in% mas_aj_2_asemat, ]
...
if (USERCONF$pakota_lataus_paikkatiedot || file.exists(PaikkaTyyppi_path)) {
    PaikkaTyyppi <- read.csv(PaikkaTyyppi_path, fileEncoding = "UTF-8-BOM") # Read from file if found or forced
} else {
...
if (FALSE) { # skip coord checks on 2nd db upload pass
...
# OVERWRITE WITH CONSTANT VALUES DETERMINED EARLIER (Pöykkö, 2024)
# (some coords have improved/changed in db, affecting the calculated coord-caused uncert)
l_dtm <- c("2.5%" = -0.407, "97.5%" = 0.407)
l_km2 <- c("2.5%" = -0.390, "97.5%" = 0.556)
...
for (i in 1:nrow(putki_yhd_temp)) {
  # Check that type is NOT monitoring pipe (incl. some moisture meters) or well
...
# Create a table where all dropped will be stored
putki_yhd_dropped  <- putki_yhd_temp[0,]
...
for (i in seq_len(nrow(link_am_man))) {
  id1 <- link_am_man[i, 1]
  id2 <- link_am_man[i, 2]
  if (!(id1 %in% kaikki_paikat$paikka_id)) {
    warning(paste("Paikka_id:tä",id1,"ei löydy paikkataulusta!"))
  }
  if (!(id2 %in% kaikki_paikat$paikka_id)) {
    warning(paste("Paikka_id:tä",id2,"ei löydy paikkataulusta!"))
  }
  if (all(!(c(id1,id2) %in% kaikki_paikat$paikka_id))) {
    warning("Putkipari ohitettu")
  } else {
...
checked <- data.table::fread(f.path(D$data,"Miralle_DRAFT_korkeudet_CHECKED.csv"))$paikka_id
...
# Write also a table which includes all alternative ground / top elevs
all_kork <-
  merge(putki_yhd_temp,
        kaikki_paikat[,c("paikka_id","yhd_yla","yhd_maa","dtm_elev","km2_elev")],
        by = "paikka_id")
all_kork["dtm_yla"] <- all_kork$dtm_elev + all_kork$putki 
all_kork["km2_yla"] <- all_kork$km2_elev + all_kork$putki

all_kork <- all_kork[stringr::str_order(all_kork$tunnus, numeric = T), ]
all_kork <- all_kork[order(all_kork$asema_tunnus), ]
data.table::fwrite(all_kork,
  check_dir(D$tulokset,"korkeudet","korkeudet_FULLEST_DATA.csv"),
  dec = ".", sep = ";", bom = T)


# Write the dropped places in a file (both db upload compliant and full infos)
putki_yhd_dropped <- putki_yhd_dropped[stringr::str_order(putki_yhd_dropped$tunnus, numeric=T), ]
putki_yhd_dropped <- putki_yhd_dropped[order(putki_yhd_dropped$asema_tunnus), ]
korkeus_dropped_out <-
  putki_yhd_dropped[ ,c("paikka_id","tunnus","asema_tunnus","asema_nimi",
                     "PaikkaTyyppi_Id","yla","yla_source","yla_verif",
                     "maa","maa_source","maa_verif","KM2_laatu")]
korkeus_dropped_out <- korkeus_dropped_out[!korkeus_dropped_out$PaikkaTyyppi_Id %in% "Lumikeppi",]
data.table::fwrite(korkeus_dropped_out,
  check_dir(D$tulokset,"korkeudet","korkeudet_tietokantaan_DROPPED.csv"),
  dec = ".", sep = ";", bom = T)

all_dropped <-
  merge(putki_yhd_dropped,
        kaikki_paikat[,c("paikka_id","yhd_yla","yhd_maa","dtm_elev","km2_elev")],
        by = "paikka_id")
all_dropped["dtm_yla"] <- all_dropped$dtm_elev + all_dropped$putki 
all_dropped["km2_yla"] <- all_dropped$km2_elev + all_dropped$putki
all_dropped <- all_dropped[stringr::str_order(all_dropped$tunnus, numeric = T), ]
all_dropped <- all_dropped[order(all_dropped$asema_tunnus), ]
data.table::fwrite(all_dropped,
  check_dir(D$tulokset,"korkeudet","korkeudet_DROPPED_FULL_DATA.csv"),
  dec = ".", sep = ";", bom = T)
