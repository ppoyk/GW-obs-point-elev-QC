...
elevtbl_sanitycheck <- function(tbl) {
  stopifnot(!anyNA(tbl$yla), !anyNA(tbl$maa)) # Elevs filled
  stopifnot(!anyNA(tbl$yla_source), !anyNA(tbl$maa_source)) # Sources filled
  stopifnot(all(tbl$maa > -5), all(tbl$maa < 1300)) # Ground elevs in [-5m, 1300m]
  stopifnot(all(tbl$yla > -5), all(tbl$yla < 1300)) # Top elevs in [-5m, 1300m]
  stopifnot(all(tbl$yla - tbl$maa > 0.1)) #Visible sections >10cm
  stopifnot(all(tbl$yla - tbl$maa < 2)) #Visible sections <2m
  stopifnot(all(tbl$yla > tbl$maa)) #All tops above ground
}
...
elevtbl_sanitycheck(putki_korkeudet_outfile)
