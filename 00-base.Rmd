...
# Pre-emptively set some known exceptions
# (Hardcoded fixes)
...
for (i in 1:nrow(hard_fixes)) { # Apply the hardcode fixes table
  putki_yhd_temp[putki_yhd_temp$paikka_id == hard_fixes[i,"val_to"], c("yla","maa")] <-
    putki_yhd_temp[putki_yhd_temp$paikka_id == hard_fixes[i,"val_from"], c("yla","maa")]
  # Unused method (the flexibility is not needed in this case)
  # test <- replace_w_key(test,"paikka_id",
  #                       toK=hard_fixes[i,"val_to"],vars=c("yla","maa"),fromK=hard_fixes[i, "val_from"])
}
rm(hard_fixes)
...
rm(a_id,m_id,p_a,p_m, new_maa,new_yla, yla_src_prio,maa_src_prio, src, diff_lim)
