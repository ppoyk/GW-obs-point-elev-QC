...
vertaa_am_man <- data.frame(matrix(nrow=nrow(link_am_man), ncol=10))#Table of discrepancies

yla_src_priority <- c("Pietari","DB","ELY","DTM+ELY","DTM+DB","DTM+MISSING",
                      "KM2+ELY","KM2+DB","KM2+MISSING")
maa_src_priority <- c("Pietari-Pietari","DB-Pietari","DB-ELY","DB","DB-MISSING",
                      "ELY","DTM","KM2")
diff_lim <- 1
...
for (i in 1:nrow(link_am_man)) {
  ...
  ,finally= {
    vertaa_am_man[i, ] <- c(p_a$tunnus, p_m$tunnus,
                            p_a$yla,p_a$yla_source, p_m$yla,p_m$yla_source, new_yla
                            p_a$maa,p_a$maa_source, p_m$maa,p_m$maa_source, new_maa)
  }
    )
...
colnames(vertaa_am_man) <- c("tunnus_AM","tunnus_man",
                             "AM_yla","AM_y_source","man_yla","man_y_source","new_yla"
                             "AM_maa","AM_m_source","man_maa","man_m_source","new_maa")
vertaa_am_man <- vertaa_am_man[!is.na(vertaa_am_man$tunnus_AM), ] #drop NA rows
rm(a_id,m_id, p_a,p_m, new_maa,new_yla, )
...
# TODO Drop also stations which have potentially other members/organisations utilizing
